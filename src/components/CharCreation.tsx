...
import { DEFAULT_PLAYER_STATS, STAT_ALLOCATION_POINTS, STAT_STEPS } from '../constants';
...
export const CharCreation: React.FC<CharCreationProps> = ({ onConfirm }) => {
  ...
  const handleRandomize = useCallback(() => {
    ...
    STAT_CONFIGS.forEach(({ key }) => {
      newStats[key] = DEFAULT_PLAYER_STATS[key];
    });
    
    // 随机分配10点
    let remaining = STAT_ALLOCATION_POINTS;
    while (remaining > 0) {
      const randomConfig = STAT_CONFIGS[Math.floor(Math.random() * STAT_CONFIGS.length)];
      const step = STAT_STEPS[randomConfig.key] || 1;
              ...
              <div key={key} className="stat-alloc-row">
                ...
                <div className="stat-controls">
                  <button 
                    ...
                    disabled={stats[key] <= DEFAULT_PLAYER_STATS[key]}
                  >
                    −
                  </button>
